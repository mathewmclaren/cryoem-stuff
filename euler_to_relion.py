...
def parse_arguments():
	...
	parser.add_argument('f_out', metavar='output.star', type=str, help='The outputted star file containing the positions and angles of your particle and the tomogram name, so Warp can extract the particles.')
	...
	parser.add_argument('--head', action='store_true', help='Instead of using the centroid of your points, use the top point(s).')
	parser.add_argument('--tail', action='store_true', help='Instead of using the centroid of your points, use the bottom (points).')
	parser.add_argument('--scale_pts', type=float, help='Scale your particle positions up by the factor provided. If unbinning, this should be original pixel size divided by binned pixel size.')
	parser.add_argument('--tomogram_star', metavar='tomograms.star', type=str, help='The path to a tomograms.star file as generated by Relion. If specified, this will generate an optimisation_set.star file for Relion 5.')
...
#	star_file = pd.DataFrame(columns=[column_name,'rlnCoordinateX','rlnCoordinateY','rlnCoordinateZ','rlnTomoSubtomogramRot','rlnTomoSubtomogramTilt','rlnTomoSubtomogramPsi'])
	...
	for file in os.listdir(modpath):
		...
		if file.endswith(".mod"): 
			print('model2point -contour -input ' + str(file) + ' -ou ' + str(file) + '.txt')
			os.system('model2point -contour -input ' + str(file) + ' -ou ' + str(file) + '.txt')
...
#			else:
#				star_file_2['rlnAngleRot'] = star_file_2['rlnTomoSubtomogramRot']
#				star_file_2['rlnAngleTilt'] = star_file_2['rlnTomoSubtomogramTilt']
#				star_file_2['rlnAnglePsi'] = star_file_2['rlnTomoSubtomogramPsi']
			if args.scale_pts:
